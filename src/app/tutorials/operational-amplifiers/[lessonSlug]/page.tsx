...
import { getOpAmpLessonBySlug, opAmpLessons, type OpAmpLesson } from '@/data/operational-amplifiers-lessons'; 
import { Breadcrumbs, type BreadcrumbItem } from '@/components/ui/breadcrumbs';
import { Card, CardContent } from '@/components/ui/card';
import { ChevronLeft, ChevronRight, Triangle as OpAmpIcon } from 'lucide-react';
...
export default function OpAmpLessonPage({ params }: LessonPageProps) {
  ...
  const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { label: 'Operational Amplifiers', href: '/tutorials/operational-amplifiers' },
    { label: lesson.title.replace(/^\d+\.\s*/, '') }, 
  ];

  const currentIndex = opAmpLessons.findIndex(l => l.slug === lesson.slug);
  ...
  const TopicIcon = lesson.Icon || OpAmpIcon;
    ...
    <article className="w-full">
      ...
      <header className="mb-8 md:mb-10 pb-6 border-b">
         <div className="flex items-center text-primary mb-3">
            ...
            <span className="text-sm font-medium tracking-wide uppercase">OPERATIONAL AMPLIFIERS</span>
        </div>
