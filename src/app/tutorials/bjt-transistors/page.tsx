    ...
    <div className="w-full">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Browse through the complete list of lessons in the BJT Transistor series.</CardDescription>
        </CardHeader>
        <CardContent>
          <ul className="space-y-2">
            {bjtTutorialLessons.map((lesson) => (
              <li key={lesson.slug}>
                <Link href={`/tutorials/bjt-transistors/${lesson.slug}`} className="block p-4 rounded-md transition-colors hover:bg-muted/50">
