...
import { getOscillatorLessonBySlug, oscillatorLessons, type OscillatorLesson } from '@/data/oscillator-lessons'; 
...
export default function OscillatorLessonPage({ params }: LessonPageProps) {
  ...
  const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { label: 'Oscillators', href: '/tutorials/oscillators' },
    { label: lesson.title.replace(/^\d+\.\s*/, '') }, 
  ];

  const currentIndex = oscillatorLessons.findIndex(l => l.slug === lesson.slug);
  const prevLesson = currentIndex > 0 ? oscillatorLessons[currentIndex - 1] : null;
  const nextLesson = currentIndex < oscillatorLessons.length - 1 ? oscillatorLessons[currentIndex + 1] : null;
  const TopicIcon = lesson.Icon || Waves;
    ...
    <article className="w-full">
      ...
      <header className="mb-8 md:mb-10 pb-6 border-b">
         <div className="flex items-center text-primary mb-3">
            <span className="text-sm font-medium tracking-wide uppercase">OSCILLATOR CIRCUITS</span>
        </div>
        ...
        <div className="flex-1 sm:flex-initial">
          {prevLesson && (
            <Button asChild variant="outline" className="w-full sm:w-auto justify-start">
              <Link href={`/tutorials/oscillators/${prevLesson.slug}`} className="flex items-center">
